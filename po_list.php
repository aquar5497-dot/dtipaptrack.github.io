...
$sql_base = "FROM purchase_orders po LEFT JOIN purchase_requests pr ON po.pr_id = pr.id";
$where_clauses = [];
$params = [];
$types = '';

if (!empty($search_query)) {
    $where_clauses[] = "(po.po_number LIKE ? OR po.supplier LIKE ? OR pr.pr_number LIKE ?)";
...
if (!empty($month_filter)) {
    $where_clauses[] = "MONTH(po.po_date) = ?";
    $params[] = $month_filter;
    $types .= 'i';
}
...
// --- PAGINATION COUNT ---
$sql_count = "SELECT COUNT(po.id) as total " . $sql_base . $where_sql;
...
// --- FINAL DATA QUERY ---
$sql = "SELECT po.*, pr.pr_number, pr.purpose " . $sql_base . $where_sql;
...
<main class="flex-1 p-4 md:p-6 bg-blue-100 min-h-screen">
  <div class="bg-white p-4 md:p-6 rounded-xl shadow-lg max-w-full">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
      <h2 class="text-xl font-bold text-gray-800">Purchase Orders (PO)</h2>

      <div class="w-full md:w-auto">
        <form method="GET" action="" class="flex flex-wrap items-center gap-2">
          <input type="search" name="search" placeholder="Search PO, Supplier, PR..." 
          ...
          <?php if ($search_query || $month_filter): ?>
              <a href="po_list.php" class="bg-gray-200 hover:bg-gray-300 px-3 py-2 rounded-lg text-sm" title="Reset">🔄</a>
          <?php endif; ?>
    ...
    <div class="relative overflow-x-auto shadow-sm border border-gray-200 rounded-lg max-h-[600px] overflow-y-auto">
      <table class="min-w-full table-auto text-sm text-left">
        <thead class="bg-gray-50 text-gray-700 font-semibold uppercase tracking-wider sticky top-0 z-10 shadow-sm">
          <tr>
            <th class="px-4 py-3 border-b bg-gray-50">PO Number</th>
            <th class="px-4 py-3 border-b bg-gray-50">PO Date</th>
            <th class="px-4 py-3 border-b bg-gray-50">Supplier</th>
            <th class="px-4 py-3 border-b bg-gray-50">Award Date</th>
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
          <?php if ($result->num_rows > 0): ?>
            <?php while($row = $result->fetch_assoc()): 
                $awardDate = $row['award_date'] ?? $row['date_of_award'] ?? $row['date_awarded'] ?? null;
                $displayAward = (!empty($awardDate) && $awardDate !== '0000-00-00') ? date('M d, Y', strtotime($awardDate)) : '—';
                $totalAmount = isset($row['total_amount']) ? number_format((float)$row['total_amount'], 2) : '0.00';
            ?>
            <tr class="hover:bg-blue-50 transition">
              ...
              <td class="px-4 py-3 text-gray-600 whitespace-nowrap">
                <?php echo !empty($row['po_date']) ? date('M d, Y', strtotime($row['po_date'])) : '—'; ?>
              </td>
              <td class="px-4 py-3 text-gray-700 max-w-xs truncate" title="<?php echo htmlspecialchars($row['supplier'] ?? ''); ?>">
                <?php echo htmlspecialchars($row['supplier'] ?? ''); ?>
              </td>
              <td class="px-4 py-3 text-gray-600"><?php echo $displayAward; ?></td>
              <td class="px-4 py-3 text-right font-bold text-blue-600 whitespace-nowrap">
                ₱<?php echo $totalAmount; ?>
              </td>
              <td class="px-4 py-3">
                <?php if ($row['pr_number']): ?>
                    <div class="flex flex-col">
                        ...
                        <span class="text-[11px] text-gray-500 truncate max-w-[150px] italic">
              ...
              <td class="px-4 py-3 text-center">
                <div class="flex items-center justify-center gap-1">
                    <a href="po_edit.php?id=<?php echo $row['id']; ?>" class="w-8 h-8 flex items-center justify-center bg-yellow-500 hover:bg-yellow-600 text-white rounded shadow-sm transition" title="Edit">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </a>
                    
                    <a href="po_delete.php?id=<?php echo $row['id']; ?>" class="w-8 h-8 flex items-center justify-center bg-gray-500 hover:bg-gray-600 text-white rounded shadow-sm transition" title="Delete" onclick="return confirm('Delete this PO?')">
          ...
          <?php else: ?>
            <tr>
              <td colspan="7" class="text-center py-10 text-gray-400 italic">No purchase orders found matching your criteria.</td>
            </tr>
          <?php endif; ?>
        </tbody>
      </table>
    ...
    <div class="px-4 py-4 flex flex-col md:flex-row items-center justify-between mt-4 border-t border-gray-100">
      ...
      <div class="flex-none flex items-center justify-center space-x-1">
        <?php
        $q_params = $_GET; unset($q_params['page']);
        $p_url = '?' . http_build_query($q_params) . '&page=';

        if ($page > 1): ?>
          <a href="<?= $p_url . ($page - 1) ?>" class="px-3 py-1 border rounded-md bg-white hover:bg-gray-100 text-gray-700 transition">Previous</a>
        <?php endif; ?>
        ...
        <?php if ($page < $total_pages): ?>
          <a href="<?= $p_url . ($page + 1) ?>" class="px-3 py-1 border rounded-md bg-white hover:bg-gray-100 text-gray-700 transition">Next</a>
        <?php endif; ?>
      </div>
